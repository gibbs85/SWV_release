...
<%
 ...
 %>
	...
	<div id="div_post_list">
		...
		<table class="post_list_nounderline">
			<td>아이디: <input type="text" name="subject" value= <%=user.getNString("id")%>/>
				<br>
				<br>
				이름: <input type="text" name="subject" value= <%=user.getNString("name")%> />
				<br>
				<br>
				email: <input type="text" name="subject" value= <%=user.getNString("email")%>  />
			</td>
		</table>
		<table class="post_list_nounderline">
		...
		<form action="ProfileMod.jsp" accept-charset="utf-8" method="get">
			<input style="width: 80px; margin-left: auto; margin-right: auto; display:block;" type="submit" id="button_comment" value="수정완료" />
		</form>
	</div>
</body>
</html>